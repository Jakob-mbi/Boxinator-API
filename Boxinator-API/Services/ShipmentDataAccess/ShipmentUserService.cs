...
namespace Boxinator_API.Services.ShipmentDataAccess
{
    public class ShipmentUserService : IShipmentUserService
    {
        ...
        public async Task<Shipment> CancelShipment(int shipmentId, string userSub)
        {
            ...
            var status = _context.Status.Where(s => s.Name == "Canceled").FirstOrDefault();
        ...
        {
            var shipments = await _context.Shipments.Include(x => x.StatusList).Where(x => x.StatusList.Any(c => c.Name == "cancelled") && x.UserSub == userSub).ToListAsync();
        ...
        {
            var shipments = await _context.Shipments.Include(x => x.StatusList).Where(x => x.StatusList.Any(c => c.Name == "completed") && x.UserSub == userSub).ToListAsync();
            return shipments != null ? shipments : throw new ShipmentNotFoundException();
        }
